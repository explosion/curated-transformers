...
def _check_toy_tokenizer(pieces):
    ...
    assert pieces.lens == [[1, 1, 1, 1, 3, 1, 1, 7, 1], [1, 3, 1, 1, 2, 4, 4, 1]]
    assert pieces.ids == [
        [
            0,
            44,
...
def _check_roberta_base_tokenizer(pieces):
    assert isinstance(pieces, PiecesWithIds)
    assert pieces.lens == [[1, 1, 1, 1, 1, 1, 1, 2, 1], [1, 1, 1, 1, 1, 2, 2, 1]]
    assert pieces.ids == [
        [0, 100, 794, 10, 1816, 19, 10, 27608, 4, 2],
        [0, 5625, 52, 40, 3529, 181, 48344, 5749, 4, 2],
    ]
    assert pieces.pieces == [
        ["<s>", "I", "Ġsaw", "Ġa", "Ġgirl", "Ġwith", "Ġa", "Ġtelescope", ".", "</s>"],
        ["<s>", "Today", "Ġwe", "Ġwill", "Ġeat", "Ġp", "okÃ©", "Ġbowl", ".", "</s>"],
    ]
